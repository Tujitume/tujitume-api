...
class GrantWalletController extends Controller
{
    ...
    public function show(int $id)
    {
        try{
            //$user_id = Auth::id();
            $wallet = GrantWallet::with(['grant'])->where('grant_id', $id)->first();
     ...
     * POST /api/v1/grant-wallets/{wallet}/deposit
     */
    //D E P O S I T  M E T H O D S
    public function deposit (Request $request, GrantWallet $wallet)
    {
        if(!$wallet){
        ...
        if($wallet->grant->user_id !== $user->id){
    ...
    public function deposit_status(Request $request, GrantWallet $wallet)
    {
        if (!Auth::check()) {
        ...
        try {
            ...
            //NotificationService
            $this->grantNotification->send('wallet.deposited', [$wallet->grant->user], [
                'amount' => $amountUsd, 'grant_title' => $wallet->grant->grant_title,
                'new_balance' => $wallet->balance, 'grant_id' => $wallet->grant_id,
            ]);
     ...
     * POST /api/v1/grant-wallets/{wallet}/reserve
     */
    public function reserve(Request $request, GrantWallet $wallet)
    {
        $validated = $request->validate([
            'amount' => 'required|numeric|min:0.01',
            'milestone_id' => 'required|exists:grant_milestones,id',
        ]);
     ...
     * POST /api/v1/grant-wallets/{wallet}/release-reserve
     */
    public function releaseReserve(Request $request, GrantWallet $wallet)
    {
        $validated = $request->validate([
            'amount' => 'required|numeric|min:0.01',
        ]);
