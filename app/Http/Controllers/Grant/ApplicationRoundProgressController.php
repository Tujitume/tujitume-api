...
class ApplicationRoundProgressController extends Controller
{
     ...
     * GET /api/v1/grant/applications/{application}/round-progress
     */
    public function index(GrantApplication $application)
    {
        try {
            ...
            $rounds = GrantRound::where('grant_id', $grant->id)
    ...
    public function publish(GrantRound $round)
    {
        $userId = auth()->id();
        ...
        if ($round->round_number > 1) {
            $previousRound = GrantRound::where('grant_id', $round->grant_id)
    ...
    public function activeApplications(GrantRound $round)
    {
    ...
    //✅ ADVANCED
    public function advancedApplications(GrantRound $round)
    {
    ...
    //✅ NOT SELECTED
    public function notSelectedApplications(GrantRound $round)
    {
        $applications = ApplicationRoundHistory::where('round_id', $round->id)
    ...
    public function autoScoreApplications(Request $request, GrantRound $round)
    {

        if ($round->grant->user_id !== auth()->id()) {
            return response()->json(['message' => 'Unauthorized.'], 403);
        }
