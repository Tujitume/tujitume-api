...
use App\Models\Grants\MilestoneAgreementComment;
...
{
    public function comment(Request $request, GrantMilestone $milestone, string $type)
    {
        $userId = auth()->id();
...
//            }
    ...
    // POST /grant/milestones/{milestone}/agreements/{type}/approve --- grant owner

    public function approve(Request $request, GrantMilestone $milestone, string $type)
    {
        $userId      = auth()->id();
        $application = $milestone->application;
        ...
        try {
            ...
            $readyField = match($type) {
                ...
                'final_approval' => 'final_approval_ready',  // ← add this field to grant_milestones if not exists
                default          => null,
            };
    ...
    // ─── Reject (grant owner rejects) ───────────────────────────────────
    // POST /grant/milestones/{milestone}/agreements/{type}/reject

    public function reject(Request $request, GrantMilestone $milestone, string $type)
    {
        $userId      = auth()->id();
        $application = $milestone->application;
    ...
    // GET /grant/milestones/{milestone}/agreements
