...
class GrantDisbursementController extends Controller
{
    protected $liprW2W;
     ...
     * GET /grant/milestones/{milestone}/disbursement-data
     */
    public function disbursementData(GrantMilestone $milestone)
    {
        $milestone->load([
            'application:id,grant_id,user_id,grant_owner_id,startup_name,contact_person_name,contact_person_email,status,funding_setup_status,awarded_amount,total_amount_requested,planning_mode',
            'application.grant:id,grant_title,grant_type,funding_per_business,mid_milestone_required',
            'application.grant.wallet:id,grant_id,balance,total_reserved,total_disbursed,status',
            'suppliers',
            'budgetItems',
            'verifications',
            'disbursements',
        ...
        return response()->json([
            ...
            'wallet' => $milestone->application->grant->wallet ?? null,
            'budget_summary' => [
    ...
    public function markCompleted(Request $request, Disbursement $disbursement)
    {
        ...
        // Authorization: only grant owner can mark as completed
        if ($disbursement->milestone->application->grant_owner_id !== $userId) {
        ...
        try {
            ...
            $this->grantNotification->send('disbursement.completed', [$disbursement->milestone->application->user, $disbursement->milestone->application->grant->owner], [
            ...
            if ($milestone->fund_release_status === 'released') {
                $this->grantNotification->send('milestone.funds_released', [$milestone->application->user], [
    ...
    public function markFailed(Request $request, Disbursement $disbursement)
    {
        ...
        // Authorization: only grant owner can mark as failed
        if ($disbursement->milestone->application->grant_owner_id !== $userId) {
            return response()->json(['error' => 'Unauthorized'], 403);
        }
        ...
        try {
            ...
            $this->grantNotification->send('disbursement.failed', [$disbursement->milestone->application->grant->owner], [
    ...
    public function reverse(Request $request, Disbursement $disbursement)
    {
        ...
        // Authorization: only grant owner can reverse
        if ($disbursement->milestone->application->grant_owner_id !== $userId) {
            return response()->json(['error' => 'Unauthorized'], 403);
        }
        ...
        try {
            ...
            $this->grantNotification->send('disbursement.reversed', [$disbursement->milestone->application->user, $disbursement->milestone->application->grant->owner], [
                'amount' => $disbursement->amount, 'supplier_name' => $disbursement->supplier->legal_name,
                'application_id' => $disbursement->milestone->app_id,
            ]);
    ...
    // Milestone Release
    public function releaseFunds(Request $request, GrantMilestone $milestone)
    {
        $userId      = auth()->id();
        ...
        try {
            ...
            DB::transaction(function () use ($milestone, $supplier, $pitch, $amountKes, &$transfer) {
                ...
                // Transactions
                $this->transaction->create($pitch->grant_owner_id, 'grant_milestone', 'lipr', $milestone->amount, $transfer['reference'], $pitch->user_id);
                $this->transaction->create($pitch->user_id, 'grant_milestone', 'lipr', $milestone->amount, $transfer['reference'], $pitch->user_id);
