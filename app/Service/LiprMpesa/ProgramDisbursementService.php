...
class ProgramDisbursementService
{
    public function __construct(
        private LiprPaymentService      $lipr,
        private ProgramNotificationService $programNotification,
    ) {}
    ...
    public function markCompleted(Disbursement $disbursement, ProgramWallet $wallet): void
    {
    ...
    public function markFailed(Disbursement $disbursement, ProgramWallet $wallet): void
    {
        DB::transaction(function () use ($disbursement, $wallet) {
    ...
    public function reverse(Disbursement $disbursement, ProgramWallet $wallet): void
    {
        ...
        // Notify
        $this->programNotification->send('disbursement.reversed', [
            $disbursement->milestone->application->program->owner
        ], [
            'amount'        => $disbursement->amount,
            'supplier_name' => $disbursement->milestoneSupplier->supplierDirectory->legal_name,
...
//        }
    ...
    public function disburse(ProgramMilestone $milestone, $amountKes, $type = 'checkout')
    {
        if($type == 'checkout'){
...
//                422);
        ...
        $validated['authorized_by'] = $milestone->application->program_owner_id;
