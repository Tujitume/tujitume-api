...
class ListingToServiceScore
{
    ...
    public function preparePivotData(array $business): array {
        return [
            'investment' => trim(str_replace(',', '', $business['y_turnover'] ?? '0-0' )),
            'sector' => strtolower(trim($business['category'])),
            'region' => strtolower(trim($business['location'])),
            'impact_areas' => array_map(fn($a) => strtolower(trim($a)), $business['social_impact_areas'] ?? [] ),
    ...
    protected function prepareServiceData($listing): array
    {
        ...
        return [
            'id' => $listing->id,
            'amount_requested' => trim($listing->price),
            //'stage' => $listing->stage,
            'impact_areas' => array_map(fn($a) => strtolower(trim($a)), $listing->social_impact_areas ?? [] ),
            'sector' => strtolower(trim($listing->category)),
            'region' => strtolower(trim($listing->location)),
    ...
    public function calculate($serviceData, $pivot)
    {
        $service = $this->prepareServiceData($serviceData);
        ...
        $ratingLikes = $this->ratingAndLikesScore($service);
    ...
    protected function investmentScore($service, $pivot) {
        ...
        return ($serviceAmount >= trim($bizMin) && $serviceAmount <= trim($bizMax)) ? 100 : 0;
    }



    protected function impactScore($service, $pivot) {
        $serviceImpacts = is_array($service['impact_areas']) ? $service['impact_areas'] : [];
