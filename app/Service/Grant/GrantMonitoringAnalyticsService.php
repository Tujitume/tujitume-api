...
class GrantMonitoringAnalyticsService
{
    public function getMeOverview(GrantApplication $app): array
    {
        $milestones = $app->grant_milestones()->get();
    ...
    public function getApplicationImpact(GrantApplication $app): array
    {
        $milestones = $app->grant_milestones()->get();
        $checkpoints = MECheckpoint::where('app_id', $app->id)
            ->with(['submission.files', 'siteVisit'])
        ...
        $operationalSummary = [
            ...
            'granted' => $disbursements->where('status', 'completed')->count(),
        ];
