    ...
    public function up(): void
    {
        Schema::create('program_milestones', function (Blueprint $table) {
            ...
            $table->enum('status', [
                ...
                'under_review',             // program owner reviewing MPRV
                ...
                'completion_approved',      // program owner signed off
                'completion_rejected',      // proof rejected, resubmission needed
            ])->default('pending');

            $table->boolean('is_template')->default(true);

            $table->enum('created_by_role', ['program_owner', 'applicant'])->default('program_owner');
            ...
            $table->foreign('app_id')
                ->references('id')->on('program_applications')->onDelete('cascade');
