<div style="max-width:1024px;margin:auto;margin-top:4rem;background-color:white;border-radius:0.5rem;box-shadow:0 4px 6px rgba(0,0,0,0.1);overflow:hidden;">
    ...
    <div style="background-color:#14532d;padding:0.9rem 0;text-align:center;color:#ffffff;">
        ...
        <h1 style="font-size:2rem;font-weight:700;margin-top:1rem;">Advanced to Next Round</h1>
    </div>
    ...
    <div style="padding:20px;font-size:14px;line-height:1.6;">
        ...
        {{-- CUSTOMISABLE: grant owner body OR blade default --}}
        @if (!empty($custom_body))
            {!! $custom_body !!}
        @else
            <p>Excellent news! You've advanced to <strong>{{ $round_name }}</strong> in <strong>{{ $grant_title }}</strong>! 🎉</p>
        ...
        <div style="text-align:center;margin-top:2rem;">
            <a href="{{ config('app.url') }}/grant/pitch/{{ $application_id }}" style="background-color:#14532d;color:white;padding:0.75rem 1.5rem;border-radius:0.5rem;text-decoration:none;font-weight:500;font-size:1rem;">View Next Round</a>
        </div>
